...
import { APPLICATION_CONFIG_URL } from '@metrics/constants'
...
export const fetchChildLocationIds = async (
  locationId: UUID,
  prefix = 'Location/'
) => {
  const children = await fetchLocationChildren(locationId)
  return children.map((child) => prefix + child.id)
}
